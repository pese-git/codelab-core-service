...
SSE (Server-Sent Events) Event Streaming обеспечивает real-time коммуникацию между сервером и клиентами для получения обновлений о состоянии агентов, выполнении задач и других событиях системы.

## Архитектура

### Компоненты

1. **SSEManager** (`app/core/sse_manager.py`)
   - Управление SSE connections
   - Broadcasting событий
   - Event buffering в Redis
   - Heartbeat механизм

2. **SSE Endpoint** (`app/routes/sse.py`)
   - `GET /my/chat/{session_id}/events/` - подключение к SSE stream
   - `GET /my/chat/stats/` - статистика SSE connections

3. **Event Schemas** (`app/schemas/event.py`)
   - `SSEEvent` - базовая схема события
   - `SSEEventType` - типы событий
...
Система поддерживает множественные SSE connections для одной сессии (например, несколько вкладок браузера).

```python
# Все connections получат событие
await sse_manager.broadcast_event(session_id, event)
```
...
sent_count = await sse_manager.broadcast_event(session_id, event)
```
...
sent_count = await sse_manager.broadcast_to_user(user_id, event)
```
...
1. **Browser limit** - Браузеры ограничивают количество одновременных SSE connections (обычно 6 per domain)
