# Руководство по развертыванию CodeLab Core Service
...
- [Production специфика](#production-специфика)
...
| Компонент | Требование |
|-----------|------------|
| CPU | 8+ cores (рекомендуется 8+ для production) |
| RAM | 32 GB (рекомендуется 32GB+) |
| Disk | 500 GB SSD (NVMe) (минимум 100GB SSD) |
| Network | 1 Gbps, стабильное интернет-соединение, рекомендуется статический IP |
| Kubernetes | 1.28+ |

### Требования ПО для Production
...
Альтернативная версия для Docker Compose:
...
### Проблема: Сервис не запускается (Docker Compose)
...
Альтернативно для Docker Compose:
...
### Проблема: Проблемы производительности (Docker Compose)
