...
## 12. Инициализация Starter Pack с LLM провайдерами

- [x] 12.1 Обновить сигнатуру `initialize_starter_pack()` - добавить параметр `llm_provider_id`
- [x] 12.2 Создать helper функцию `get_or_create_user_default_provider()` для получения/создания default провайдера
- [x] 12.3 Обновить логику создания агентов в `initialize_starter_pack()` - устанавливать `llm_provider_id`
- [x] 12.4 Обновить `app/config.py` - добавить переменные для default LLM (model, embedding_model, base_url, api_key)
- [x] 12.5 Обновить вызовы функции в `app/routes/projects.py` и `scripts/init_db.py`
- [x] 12.6 Обновить тесты в `test_create_project_with_starter_pack.py` - проверять llm_provider_id

## 13. Тестирование и валидация

- [ ] 13.1 Запустить все unit тесты - проверить 100% coverage новых модулей
- [ ] 13.2 Запустить все интеграционные тесты
- [ ] 13.3 Запустить API тесты через pytest
- [x] 13.4 Проверить линтинг через ruff (все ошибки исправлены)
- [ ] 13.5 Проверить type hints через mypy (если используется)
- [ ] 13.6 Написать и запустить smoke тесты на dev окружении
- [ ] 13.7 Проверить edge cases: обработка ошибок, таймауты, невалидные ключи

## 14. Финальные проверки

- [ ] 14.1 Убедиться, что миграции применяются корректно
- [ ] 14.2 Убедиться, что все endpoints работают и возвращают корректные коды статуса
- [ ] 14.3 Убедиться, что API ключи НЕ логируются и НЕ хранятся в PostgreSQL
- [ ] 14.4 Убедиться, что изоляция пользователей работает корректно
- [ ] 14.5 Убедиться, что все операции логируются в audit log
- [ ] 14.6 Провести код-ревью
- [ ] 14.7 Обновить документацию по завершении
