...
Publisher ДОЛЖЕН использовать механизмы, безопасные для нескольких воркеров.

#### Scenario: SKIP LOCKED (предпочтительно) или Optimistic Locking
- **WHEN** несколько экземпляров publisher работают параллельно
- **THEN** выборка pending событий должна либо:
  - Использовать `FOR UPDATE SKIP LOCKED` для пессимистической блокировки (PostgreSQL 9.5+), либо
  - Использовать `batch_id` для распределения батчей между воркерами (optimistic approach), либо
  - Использовать distributed lock через Redis
- **AND** не обрабатывает одну запись дважды одновременно
- **NOTE**: Текущая реализация использует простую SELECT без блокировки. Для production с множественными воркерами рекомендуется реализовать одно из указанных выше
