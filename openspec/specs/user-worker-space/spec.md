...
#### Scenario: Tool получает путь к файлу как параметр
- **WHEN** tool вызывается агентом для работы с файлом
- **THEN** tool получает путь к файлу как параметр (например, `/path/to/project/file.py`) и выполняет операцию в контексте пользователя
...
Worker Space ДОЛЖЕН явно определять граничные условия безопасности и запрещать опасные операции.
...
## 6. Архитектура доступа к workspace и документирование

### 6.1 Документирование архитектурного потока
...
### 6.2 Разделение ответственности компонентов
