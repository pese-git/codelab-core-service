...
class WorkerSpaceManager:
    """Manager for user worker spaces."""

    def __init__(self, agent_bus: AgentBus):
        """Initialize worker space manager.

        Args:
            agent_bus: Agent bus instance
        """
        self.agent_bus = agent_bus
        self.spaces: dict[str, UserWorkerSpace] = {}  # key: f"{user_id}_{project_id}"
        self.lock = asyncio.Lock()
    ...
    async def get(self, user_id: UUID, project_id: str) -> Optional[UserWorkerSpace]:
        """Get worker space if exists.
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "spaces": {
                key: space.get_agent_stats() for key, space in self.spaces.items()
            },
        }
