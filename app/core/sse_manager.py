"""SSE Manager for managing Server-Sent Events connections."""
...
class SSEManager:
    ...
    def __init__(self, redis: Redis):
        self.redis = redis
        # connections[session_id] = [SSEConnection, ...]
        self.connections: dict[UUID, list[SSEConnection]] = defaultdict(list)
    ...
    async def start(self) -> None:
        """Start SSE manager background tasks."""
        if self._heartbeat_task is None:
            self._heartbeat_task = asyncio.create_task(self._heartbeat_loop())
            logger.info("SSE Manager started")

    async def stop(self) -> None:
        """Stop SSE manager and close all connections."""
    ...
    async def register_connection(
        ...
        connection = SSEConnection(session_id, user_id, queue)
        ...
        logger.info(
            f"SSE connection registered: user={user_id}, session={session_id}, "
    ...
    async def unregister_connection(
        ...
        logger.info(
            f"SSE connection unregistered: user={user_id}, session={session_id}"
        )

    async def broadcast_event(
        self, session_id: UUID, event: SSEEvent, buffer: bool = True
    ) -> int:
        """
    ...
    async def broadcast_to_user(
        self, user_id: UUID, event: SSEEvent, buffer: bool = True
    ) -> int:
        """
    ...
    async def _buffer_event(self, session_id: UUID, event: SSEEvent) -> None:
    ...
    async def _send_buffered_events(
        self, session_id: UUID, connection: SSEConnection
    ) -> None:
